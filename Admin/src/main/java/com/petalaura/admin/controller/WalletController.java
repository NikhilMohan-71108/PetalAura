...
public class WalletController {
    ...
    public String getWalletDetails(@PathVariable Long id, Principal principal, Model model, Authentication authentication) {
        if (principal == null) {
            return "redirect:/login";
        }
        Customer customer = customerService.findById(id);
        // Retrieve customer by email (logged-in user)
     //   Customer customer = customerService.findByEmail(principal.getName());
        ...
        model.addAttribute("wallet", wallet); // Add wallet object
        model.addAttribute("walletHistoryList", walletHistoryDtoList); // Add wallet history list

        return "wallet"; // Return the wallet view template
    }
...
//        }
    ...
    @ResponseBody
    public String verifyWalletPayments(@RequestBody Map<String,Object> data, HttpSession session, Principal principal) throws RazorpayException {
